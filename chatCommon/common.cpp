                      ...
                      const char *apMsg)
{
    ...
    std::cerr << apFile << ":" << apLine << ":" << " error: assertion failed (" << apExpr << ") in "
              << apFunc << "() with message \"" << apMsg << "\"\n";
    std::abort();
}
} // namespace SQLite
#endif
...
int main()
{
    ...
    fmt::print(fmt::fg(fmt::color::green), "SQliteC++ version {}\n", SQLITECPP_VERSION);
    ...
    try {
        ...
        while (query.executeStep()) {
            std::cout << "row (" << query.getColumn(0).getInt() << ", \"" << query.getColumn(1)
                      << "\")\n";
        }
